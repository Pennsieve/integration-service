...
CREATE TABLE IF NOT EXISTS notifications.subscriptions (
    ...
    user_id         INTEGER     NOT NULL REFERENCES notifications.users (user_id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS notifications.notifications (
    ...
    topic_id        INTEGER     NOT NULL REFERENCES notifications.topics (topic_id) ON DELETE CASCADE,
    sender_id       INTEGER     NOT NULL REFERENCES notifications.users (user_id),
    title           TEXT        NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_notifications_notifications_topic_id   ON notifications.notifications (topic_id);
...
CREATE TABLE IF NOT EXISTS notifications.messages (
    ...
    from_user       INTEGER     NOT NULL REFERENCES notifications.users (user_id),
    to_user         INTEGER     NOT NULL REFERENCES notifications.users (user_id),
...
CREATE TABLE IF NOT EXISTS notifications.notification_preferences (
    user_id       INTEGER PRIMARY KEY REFERENCES notifications.users (user_id) ON DELETE CASCADE,
