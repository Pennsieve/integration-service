...
# Create Account Creation Cloudwatch DLQ Alarm
resource "aws_cloudwatch_metric_alarm" "webhook_integration_dlq_cloudwatch_metric_alarm" {
  alarm_name                = "${var.environment_name}-webhook-integration-deadletter-queue-alarm-${data.terraform_remote_state.region.outputs.aws_region_shortname}"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "1"
  metric_name               = "ApproximateNumberOfMessagesVisible"
  namespace                 = "AWS/SQS"
  ...
  dimensions = {
    QueueName = aws_sqs_queue.webhook_integration_deadletter_queue.name
  }
}